@echo off
REM ============================================================
REM  Secora - expose the app over a free Cloudflare Tunnel
...
start "Secora App" cmd /k "php artisan serve --host 127.0.0.1 --port 8000"

REM Give the app a moment to boot.
timeout /t 3 >nul

REM 2) Cloudflare Tunnel -> the app. The URL prints in this new window.
start "Secora Tunnel" cmd /k "C:\xampp\cloudflared\cloudflared.exe tunnel --url http://127.0.0.1:8000"

echo.
echo Two windows opened:
echo   - "Secora App"    : the Laravel server (leave it running)
echo   - "Secora Tunnel" : shows your https://<random>.trycloudflare.com URL
echo.
